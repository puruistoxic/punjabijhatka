...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		echo $prepend . $before . esc_html__( 'Products tagged &ldquo;', 'SPICE' ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . esc_html__( 'Search results for &ldquo;', 'SPICE' ) . get_search_query() . '&rdquo;' . $after;
		...
		echo $before . esc_html__( 'Error 404', 'SPICE' ) . $after;
		...
		echo $before . esc_html__( 'Search results for &ldquo;', 'SPICE' ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			echo $before . esc_html__( 'Posts tagged &ldquo;', 'SPICE' ) . single_tag_title('', false) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata($author);
		echo $before . esc_html__( 'Author:', 'SPICE' ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) )
		echo ' (' . esc_html__( 'Page', 'SPICE' ) . ' ' . get_query_var( 'paged' ) . ')';
