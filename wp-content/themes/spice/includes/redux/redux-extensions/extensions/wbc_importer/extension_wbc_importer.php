    ...
    class ReduxFramework_extension_wbc_importer {
        ...
        public function __construct( $parent ) {
            ...
            if ( empty( $this->extension_dir ) ) {
                $this->extension_dir = trailingslashit( str_replace( '\\', '/', trailingslashit( get_template_directory()).'includes/redux/redux-extensions/extensions/wbc_importer' ) );
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
                ...
                if ( file_exists( $demo_data_loc.'/'.$import_parts['content_file'] ) && is_file( $demo_data_loc.'/'.$import_parts['content_file'] ) ) {
                    ...
                    if ( !isset( $import_parts['imported'] ) || true === $reimporting ) 
                    {
                        ...
                        /***** SETTING HOME PAGE MENU ***/
                        $menu=get_terms( 'nav_menu', array( 'hide_empty' => true,'slug'=>'home-page-block' ) ); 
                        $locations['homepage-block'] = $menu[0]->term_id;
                        /***** SETTING PRIMARY MENU **/
                        $primarymenu=get_terms( 'nav_menu', array( 'hide_empty' => true,'slug'=>'header-style-three' ) ); 
                        $locations['primary-menu'] = $primarymenu[0]->term_id;                        
                    ...
                    {
                        echo esc_html__( "Demo Already Imported", 'SPICE' );
                    }
                }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', esc_html__( 'Demo Importer', 'SPICE' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : esc_html__( 'Demo Importer', 'SPICE' );

            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'It will import the demo contents and set the HOME page [ Please install the required Plugins before doing it ]', 'SPICE' ) ).'</p>',
