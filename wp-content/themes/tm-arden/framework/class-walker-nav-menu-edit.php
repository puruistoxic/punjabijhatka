...
class Insight_Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'tm-arden' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__( '%s (Pending)', 'tm-arden' ), $item->title );
		}
	...
	<li id="menu-item-<?php echo esc_attr( $item_id ); ?>" class="<?php echo implode( ' ', $classes ); ?>">
		<div class="menu-item-bar">
			<div class="menu-item-handle">
				<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span
						class="is-submenu" <?php echo '' . $submenu_text; ?>><?php esc_html_e( 'sub item', 'tm-arden' ); ?></span></span>
				<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"
							   aria-label="<?php esc_attr_e( 'Move up', 'tm-arden' ) ?>">&#8593;</a>
							|
							<a href="<?php
							echo wp_nonce_url( add_query_arg( array(
							...
							?>" class="item-move-down" aria-label="<?php esc_attr_e( 'Move down', 'tm-arden' ) ?>">&#8595;</a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" href="<?php
						echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"
						   aria-label="<?php esc_attr_e( 'Edit menu item', 'tm-arden' ); ?>"><?php esc_html_e( 'Edit', 'tm-arden' ); ?></a>
					</span>
			</div>
		</div>
		...
		<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
			<?php if ( 'custom' == $item->type ) : ?>
				<p class="field-url description description-wide">
					<label for="edit-menu-item-url-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'URL', 'tm-arden' ); ?><br/>
			...
			<p class="description description-wide">
				<label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Navigation Label', 'tm-arden' ); ?><br/>
			...
			<p class="field-title-attribute field-attr-title description description-wide">
				<label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Title Attribute', 'tm-arden' ); ?><br/>
			...
			<p class="field-link-target description">
				<label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
					...
					<?php esc_html_e( 'Open link in a new tab', 'tm-arden' ); ?>
				</label>
			</p>
			<p class="field-css-classes description description-thin">
				<label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'CSS Classes (optional)', 'tm-arden' ); ?><br/>
			...
			<p class="field-xfn description description-thin">
				<label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Link Relationship (XFN)', 'tm-arden' ); ?><br/>
			...
			<p class="field-description description description-wide">
				<label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Description', 'tm-arden' ); ?><br/>
					...
					<span
						class="description"><?php esc_html_e( 'The description will be displayed in the menu if the current theme supports it.', 'tm-arden' ); ?></span>
				</label>
			</p>
			...
			<p class="field-feature description">
				<label for="edit-menu-item-feature-<?php echo esc_attr( $item_id ); ?>">
					...
					<?php esc_html_e( 'Mark this item as feature item', 'tm-arden' ); ?>
				</label>
			</p>
			<p class="field-image-hover description description-wide">
				<label
					for="edit-menu-item-image-hover-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Image on hover', 'tm-arden' ); ?></label>
					...
					<div class="kungfu-media-image">
						<?php if ( $item->image_hover ) : ?>
							<img src="<?php echo esc_url( $item->image_hover_url ); ?>"
							     alt="<?php esc_attr__( 'Image Hover', 'tm-arden' ) ?>">
						<?php endif; ?>
					</div>
			<p>
			...
			<a class="kungfu-media-open kungfu-button success">
				<i class="fa fa-upload"></i><?php esc_html_e( 'Upload', 'tm-arden' ); ?>
			</a>
			<a class="kungfu-media-remove kungfu-button danger"
				...
				<?php endif; ?>
			>
				<i class="fa fa-trash-o"></i><?php esc_html_e( 'Remove', 'tm-arden' ); ?>
			</a>
		</div>
		</p>
		...
		<p class="field-move hide-if-no-js description description-wide">
			<label>
				<span><?php esc_html_e( 'Move', 'tm-arden' ); ?></span>
				<a href="#" class="menus-move menus-move-up"
				   data-dir="up"><?php esc_html_e( 'Up one', 'tm-arden' ); ?></a>
				<a href="#" class="menus-move menus-move-down"
				   data-dir="down"><?php esc_html_e( 'Down one', 'tm-arden' ); ?></a>
				<a href="#" class="menus-move menus-move-left" data-dir="left"></a>
				<a href="#" class="menus-move menus-move-right" data-dir="right"></a>
				<a href="#" class="menus-move menus-move-top"
				   data-dir="top"><?php esc_html_e( 'To the top', 'tm-arden' ); ?></a>
			</label>
		</p>
		...
		<div class="menu-item-actions description-wide submitbox">
			<?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
				<p class="link-to-original">
					<?php printf( esc_html__( 'Original: %s', 'tm-arden' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
				</p>
			<?php endif; ?>
			...
			echo wp_nonce_url( add_query_arg( array(
				...
				'menu-item' => $item_id,
			), admin_url( 'nav-menus.php' ) ), 'delete-menu_item_' . $item_id ); ?>"><?php esc_html_e( 'Remove', 'tm-arden' ); ?></a>
			<span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js"
			                                                   ...
			                                                   ?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Cancel', 'tm-arden' ); ?></a>
		</div>
