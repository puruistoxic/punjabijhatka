	...
	class Insight_Post_Like {
		...
		public function process_simple_like() {
			...
			if ( ! wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
				exit( esc_html__( 'Not permitted', 'tm-arden' ) );
			}
		...
		public function get_simple_likes_button( $post_id, $is_comment = null ) {
			...
			if ( $this->already_liked( $post_id, $is_comment ) ) {
				...
				$title = esc_html__( 'Unlike', 'tm-arden' );
				$icon  = $icon_full;
			} else {
				$class = '';
				$title = esc_html__( 'Like', 'tm-arden' );
				$icon  = $icon_empty;
			}
		...
		function show_user_likes( $user ) { ?>
			<table class="form-table">
				<tr>
					<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'tm-arden' ); ?></label></th>
					<td>
						<?php
						...
						<?php else : ?>
							<p><?php esc_html_e( 'You do not like anything yet.', 'tm-arden' ); ?></p>
							<?php
						endif;
						wp_reset_postdata();
						?>
