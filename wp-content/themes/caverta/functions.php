...
function caverta_pagenavi( $p = 2 ) { // pages will be show before and after current page
  ...
  echo '</span>';
  if ( $paged > $p + 1 ) caverta_p_link( 1, esc_html__('First', 'caverta') );
  if ( $paged > $p + 2 ) echo '... ';
  for( $i = $paged - $p; $i <= $paged + $p; $i++ ) { // Middle pages
    if ( $i > 0 && $i <= $max_page ) $i == $paged ? print "<span class='page-numbers current-page'>{$i}</span> " : caverta_p_link( $i );
  }
  if ( $paged < $max_page - $p - 1 ) echo '... ';
  if ( $paged < $max_page - $p ) caverta_p_link( $max_page, esc_html__('Last', 'caverta') );
  echo '<span class="nav-page">';
  next_posts_link(esc_html__('Older &rarr;', 'caverta'));
  echo '</span>';
  echo '</div><!--end-->';
}
function caverta_p_link( $i, $title = '' ) {
...
//Search filter
function caverta_SearchFilter($query) {
...
add_filter('pre_get_posts','caverta_SearchFilter');

//Limit number of tags inside widget
function caverta_tag_widget_limit($args){
...
add_filter('widget_tag_cloud_args', 'caverta_tag_widget_limit');