...
if ( ! class_exists( 'InsightCore' ) ) {
	class InsightCore {
		...
		function __construct() {
			...
			// Dashboard
			include_once( INSIGHT_CORE_INC_DIR . '/dashboard/index.php' );
		}
		...
		// Check if has changelogs file
		public static function has_changelogs() {
			if ( false === ( $changelogs = get_transient( 'ic_changelogs' ) ) ) {
		...
		public static function check_theme_update() {
			...
			$has_update  = false;
			if ( self::$info['api'] ) {
				if ( $updates = self::has_changelogs() ) {
					if ( is_array( $updates ) ) {
